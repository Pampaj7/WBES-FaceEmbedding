...
SCRIPT=/deck/datasets/WBES-FaceEmbedding/face_embedding/gt_encdec/remeshing/intrinsic/perturbated/compare_model_vs_nicp_correspondence_topology_breakdown.py

OUT_DIR=$ROOT/perturbation_ranking_vs_nicp_correspondence_topology_breakdown/best_by_xtopo_mesh_clean_split-eval_pairs-cross_topology_topopairs-cross_only_ordered-1_subjects-100_meshes-10_scenarios-clean-jitter-translation-rotation-mixed_icp128_nicp128_iter8
QUEUE_LOG=$ROOT/nicp_correspondence_queue.log
...
echo "[$(timestamp)] Starting nICP-correspondence topology breakdown"
"$PYTHON" "$SCRIPT" \
  --model_path "$CKPT" \
  --data_dir "$DATA_DIR" \
  --dist_npz "$DIST_NPZ" \
...
echo "[$(timestamp)] nICP-correspondence topology breakdown finished"
