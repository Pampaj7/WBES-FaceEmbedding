...
RANK_OUT=$ROOT/faceverse_ranking_vs_gt_neutral_full_mixed_xtopo_9a81466d_best_by_xtopo_mesh_clean_postperturb_icp
SIGMA_OUT=$ROOT/faceverse_ranking_vs_gt_sigma_sweep_neutral_full_mixed_xtopo_9a81466d_best_by_xtopo_mesh_clean_postperturb_icp
QUEUE_LOG=$ROOT/mixed_xtopo_faceverse_postperturb_icp_queue.log
...
echo "[$(timestamp)] Starting FaceVerse ranking benchmark with post-perturb ICP"
"$PYTHON" "$RANK_SCRIPT" \
  "${common_args[@]}" \
  --out_dir "$RANK_OUT" \
...
echo "[$(timestamp)] Starting FaceVerse sigma sweep with post-perturb ICP"
"$PYTHON" "$SIGMA_SCRIPT" \
  "${common_args[@]}" \
  --out_dir "$SIGMA_OUT" \
...
echo "[$(timestamp)] FaceVerse post-perturb ICP pipeline finished"
