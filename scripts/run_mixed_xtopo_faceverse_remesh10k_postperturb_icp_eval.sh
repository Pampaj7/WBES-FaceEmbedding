...
DATA_DIR=$ROOT/cross_topology_10k_with_ops

RANK_OUT=$ROOT/faceverse_xtopo10k_ranking_vs_gt_mixed_xtopo_9a81466d_best_by_xtopo_mesh_clean_postperturb_icp
SIGMA_OUT=$ROOT/faceverse_xtopo10k_sigma_sweep_vs_gt_mixed_xtopo_9a81466d_best_by_xtopo_mesh_clean_postperturb_icp
QUEUE_LOG=$ROOT/mixed_xtopo_faceverse_remesh10k_postperturb_icp_queue.log
...
if [[ ! -d "$DATA_DIR" ]]; then
  echo "Missing cross-topology FaceVerse data dir: $DATA_DIR" >&2
  exit 1
fi

echo "[$(timestamp)] Starting FaceVerse cross-topology ranking benchmark with post-perturb ICP"
"$PYTHON" "$RANK_SCRIPT" \
  "${common_args[@]}" \
  --out_dir "$RANK_OUT" \
...
echo "[$(timestamp)] Starting FaceVerse cross-topology sigma sweep with post-perturb ICP"
"$PYTHON" "$SIGMA_SCRIPT" \
  "${common_args[@]}" \
  --out_dir "$SIGMA_OUT" \
...
echo "[$(timestamp)] FaceVerse remesh10k cross-topology post-perturb ICP pipeline finished"
