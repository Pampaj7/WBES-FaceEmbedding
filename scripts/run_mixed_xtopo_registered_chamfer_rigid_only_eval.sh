...
OUT_DIR=$ROOT/perturbation_ranking_vs_registered_chamfer_topology_breakdown/best_by_xtopo_mesh_clean_split-eval_pairs-cross_topology_topopairs-cross_only_ordered-1_subjects-100_meshes-10_scenarios-clean-jitter-translation-rotation-mixed_icp128_rigidonly
QUEUE_LOG=$ROOT/registered_chamfer_rigid_only_queue.log
...
echo "[$(timestamp)] Starting rigid-only registered-chamfer topology breakdown"
"$PYTHON" "$SCRIPT" \
  --model_path "$CKPT" \
  --data_dir "$DATA_DIR" \
  --dist_npz "$DIST_NPZ" \
...
echo "[$(timestamp)] Rigid-only registered-chamfer topology breakdown finished"
