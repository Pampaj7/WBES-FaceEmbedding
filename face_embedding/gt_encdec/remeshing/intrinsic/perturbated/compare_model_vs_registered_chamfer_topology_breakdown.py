...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            ...
            "rigid ICP + non-rigid CPD registration, broken down by topology-pair."
        )
    )
    ...
    p.add_argument("--chamfer_batch_pairs", type=int, default=8, help="Pair batch size for registered Chamfer")
    ...
    p.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Refine rigid ICP with non-rigid CPD before Chamfer",
    )
    p.add_argument("--cpd_points", type=int, default=64, help="Vertices sampled from each mesh for non-rigid CPD")
    p.add_argument("--cpd_alpha", type=float, default=2.0, help="CPD regularization strength")
    p.add_argument("--cpd_beta", type=float, default=0.2, help="CPD Gaussian-kernel width")
    p.add_argument("--cpd_max_iteration", type=int, default=15, help="CPD max iterations")
    p.add_argument("--cpd_tolerance", type=float, default=1.0e-4, help="CPD convergence tolerance")
    p.add_argument("--cpd_outlier_weight", type=float, default=0.05, help="CPD outlier weight")
    p.add_argument("--warp_chunk_size", type=int, default=4096, help="Chunk size when applying the CPD warp")
    return p.parse_args()
...
def _build_default_out_dir(
    ...
    slug = base.slugify_token(
        ...
        f"icppts-{int(cli_args.icp_points)}_cpdpts-{int(cli_args.cpd_points)}_"
        f"cpd-{int(bool(cli_args.use_nonrigid_cpd))}"
    )
    return run_dir / "perturbation_ranking_vs_registered_chamfer_topology_breakdown" / slug
...
def _summarize_subset(
    *,
    ...
    registered_chamfer_mesh_values: np.ndarray,
    ...
    if n_mesh_pairs == 0:
        return {
            ...
            "registered_chamfer_spearman": float("nan"),
            "registered_chamfer_pearson": float("nan"),
            "delta_spearman": float("nan"),
            "delta_pearson": float("nan"),
            "model_beats_registered_chamfer": False,
            "n_subjects": 0,
            "n_subject_pairs": 0,
            "n_mesh_pairs": 0,
            "n_observations": 0,
            ...
            "nonrigid_cpd_seconds_mean": float("nan"),
            "nonrigid_cpd_seconds_median": float("nan"),
            "warp_seconds_mean": float("nan"),
            "warp_seconds_median": float("nan"),
            "registration_seconds_mean": float("nan"),
            "registration_seconds_median": float("nan"),
    ...
    registered_chamfer_mesh = np.asarray(registered_chamfer_mesh_values[selected_indices], dtype=np.float64)

    if mesh_pair_level:
        gt_values = gt_mesh_values
        latent_values = latent_mesh
        registered_chamfer_values = registered_chamfer_mesh
        ...
        registered_chamfer_values = _aggregate_grouped_values(registered_chamfer_mesh, group_members=group_members)
    ...
    registered_chamfer_spearman = float(base.spearman_corr(gt_values, registered_chamfer_values))
    registered_chamfer_pearson = float(base.pearson_corr(gt_values, registered_chamfer_values))
    delta_spearman = float(latent_spearman - registered_chamfer_spearman)
    delta_pearson = float(latent_pearson - registered_chamfer_pearson)
    ...
    row = {
        ...
        "registered_chamfer_spearman": registered_chamfer_spearman,
        "registered_chamfer_pearson": registered_chamfer_pearson,
        "delta_spearman": delta_spearman,
        "delta_pearson": delta_pearson,
        "model_beats_registered_chamfer": bool(latent_spearman > registered_chamfer_spearman),
...
def _summary_header() -> List[str]:
    return [
        ...
        "mesh_pair_level",
        "latent_spearman",
        "registered_chamfer_spearman",
        "delta_spearman",
        "latent_pearson",
        "registered_chamfer_pearson",
        "delta_pearson",
        "model_beats_registered_chamfer",
        "n_subjects",
        "n_subject_pairs",
        "n_mesh_pairs",
        "n_observations",
        ...
        "nonrigid_cpd_seconds_mean",
        "nonrigid_cpd_seconds_median",
        "warp_seconds_mean",
        "warp_seconds_median",
        "registration_seconds_mean",
        "registration_seconds_median",
            ...
            writer.writerow(
                {
                    ...
                    "registered_chamfer_spearman": _format_float(row["registered_chamfer_spearman"]),
                    "delta_spearman": _format_float(row["delta_spearman"]),
                    "latent_pearson": _format_float(row["latent_pearson"]),
                    "registered_chamfer_pearson": _format_float(row["registered_chamfer_pearson"]),
                    "delta_pearson": _format_float(row["delta_pearson"]),
                    "model_beats_registered_chamfer": int(bool(row["model_beats_registered_chamfer"])),
                    ...
                    "nonrigid_cpd_seconds_mean": _format_float(row["nonrigid_cpd_seconds_mean"]),
                    "nonrigid_cpd_seconds_median": _format_float(row["nonrigid_cpd_seconds_median"]),
                    "warp_seconds_mean": _format_float(row["warp_seconds_mean"]),
                    "warp_seconds_median": _format_float(row["warp_seconds_median"]),
                    "registration_seconds_mean": _format_float(row["registration_seconds_mean"]),
                    "registration_seconds_median": _format_float(row["registration_seconds_median"]),
...
def _write_pair_summary_md(path: Path, row: dict) -> None:
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Registered Chamfer Topology Pair Breakdown\n\n")
        f.write(
            "| Scenario | Pair Label | Mesh-level | Lat Sp | Reg Chamfer Sp | Delta Sp | "
            "Lat Pe | Reg Chamfer Pe | Delta Pe | Rigid mean s | Nonrigid mean s | Total mean s |\n"
        )
        f.write("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |\n")
        f.write(
            "| "
...
def _build_registration_index_cache(
    *,
    ...
    for record in sample_records:
        ...
        cpd_cache[dataset_idx] = reg_utils.build_sample_vertex_indices(
            vertex_count=n_verts,
            n_points=int(cpd_points),
            seed=int(base_seed) * 1_000_003 + dataset_idx * 97 + 23,
        )
    return icp_cache, cpd_cache
...
def _build_point_sets_for_scenario(
    *,
    ...
    cpd_index_cache: Dict[int, np.ndarray],
    ...
    for local_idx, record in enumerate(sample_records):
        ...
        cpd_point_sets.append(reg_utils.extract_point_subset(V, indices=cpd_index_cache[dataset_idx]))
    return icp_point_sets, cpd_point_sets
...
def main() -> None:
    ...
    print(
        f"Prepared registered-chamfer subset: subjects={len(active_subjects)} "
    ...
    print(
        f"Prepared registered-chamfer pair context: subject_pairs={pair_ctx.subject_pair_count} "
        f"mesh_pairs={pair_ctx.mesh_pair_count}",
        flush=True,
    )

    icp_index_cache, cpd_index_cache = _build_registration_index_cache(
        dataset=dataset,
        sample_records=pair_ctx.sample_records,
        sample_cache=sample_cache,
        icp_points=int(cli_args.icp_points),
        cpd_points=int(cli_args.cpd_points),
    ...
    metadata = {
        ...
        "registration": {
            ...
            "use_nonrigid_cpd": bool(cli_args.use_nonrigid_cpd),
            "cpd_points": int(cli_args.cpd_points),
            "cpd_alpha": float(cli_args.cpd_alpha),
            "cpd_beta": float(cli_args.cpd_beta),
            "cpd_max_iteration": int(cli_args.cpd_max_iteration),
            "cpd_tolerance": float(cli_args.cpd_tolerance),
            "cpd_outlier_weight": float(cli_args.cpd_outlier_weight),
            "warp_chunk_size": int(cli_args.warp_chunk_size),
    ...
    for scenario_index, scenario in enumerate(scenarios):
        print(
            f"Starting registered-chamfer scenario {scenario_index + 1}/{len(scenarios)}: {scenario.name}",
            flush=True,
        )
        ...
        icp_point_sets, cpd_point_sets = _build_point_sets_for_scenario(
            vertex_sets=vertex_sets,
            sample_records=pair_ctx.sample_records,
            icp_index_cache=icp_index_cache,
            cpd_index_cache=cpd_index_cache,
        )
        print(
            f"Scenario {scenario.name}: computing registered Chamfer over {pair_ctx.mesh_pair_count} mesh pairs "
            f"(batch_pairs={int(cli_args.chamfer_batch_pairs)})",
            flush=True,
        )
        registered_chamfer_mesh_values, timing_arrays = reg_utils.compute_pairwise_registered_chamfer_values(
            vertex_sets=vertex_sets,
            ...
            cpd_outlier_weight=float(cli_args.cpd_outlier_weight),
            warp_chunk_size=int(cli_args.warp_chunk_size),
            progress_desc=f"{scenario.name} registered chamfer pairs",
            show_progress=True,
        )
        print(
            f"Scenario {scenario.name}: finished registered Chamfer over {pair_ctx.mesh_pair_count} mesh pairs",
            flush=True,
        )
        ...
        if cli_args.save_pair_timings:
            np.savez(
                ...
                nonrigid_cpd_seconds=np.asarray(timing_arrays["nonrigid_cpd_seconds"], dtype=np.float64),
                warp_seconds=np.asarray(timing_arrays["warp_seconds"], dtype=np.float64),
                registration_seconds=np.asarray(timing_arrays["registration_seconds"], dtype=np.float64),
        ...
        scenario_overall = _summarize_subset(
            ...
            registered_chamfer_mesh_values=registered_chamfer_mesh_values,
        ...
        print(
            ...
            f"reg_chamfer_sp={scenario_overall['registered_chamfer_spearman']:.4f} "
