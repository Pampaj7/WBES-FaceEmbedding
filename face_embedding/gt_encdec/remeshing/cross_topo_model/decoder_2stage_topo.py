...
# ======
VARIANT_RE = re.compile(r"^(id\d+)_")

def build_subject_map(files):
    subj_to_idxs = {}
    for idx, f in enumerate(files):
        name = os.path.basename(f)
        m = VARIANT_RE.match(name)
        subj = m.group(1) if m else name.split("_")[0]
        subj_to_idxs.setdefault(subj, []).append(idx)
    return subj_to_idxs

def subset_from_subjects(dataset, subjects):
    subj_map = build_subject_map(dataset.files)
    idxs = []
    for s in subjects:
        idxs.extend(subj_map.get(s, []))
    return Subset(dataset, sorted(idxs))
...
def main():
    ...
    ENCODER_CKPT = "encoder_stage1_multitopo_second_try/encoder_stage1_epoch50_noeval.pth"  
    ...
    val_subjects = set(rng.choice(subjects, size=n_val, replace=False))
    train_subjects = [s for s in subjects if s not in val_subjects]

    train_set = subset_from_subjects(dataset, train_subjects)
    val_set   = subset_from_subjects(dataset, val_subjects)

    print(f"Train subjects: {len(train_subjects)} | meshes: {len(train_set)}")
    print(f"Val   subjects: {len(val_subjects)} | meshes: {len(val_set)}")
