...
from scipy.sparse.linalg import spsolve, cg, splu, lsqr, lsmr
...
class LandmarkBasedElasticAligner(BaseNonrigidAligner):
    ...
    def align(self, R, G, Glmks, lmk_indices):
        ...
        Dxl, Dyl, Dzl = Dx[lmk_indices[self.sel_lmk_ids], :], Dy[lmk_indices[self.sel_lmk_ids], :], Dz[lmk_indices[
                                                                                                           self.sel_lmk_ids],
                                                                                                    :]
        ...
        # 🚀 Solve optimization problem with stability fixes
        # dxu, dyu, dzu = self.solve_optimization_problem_sequential(Dx, Dxl, bxl, Dyl, byl, Dzl, bzl)
    ...
    def solve_optimization_problem(self, D, Dl, b):
        np.random.seed(1907)  # Keeping their original seed for consistency
        m = Dl.shape[0]
        n = Dl.shape[1]
...
class NonrigidICPAligner(BaseNonrigidAligner):
    ...
    def spsolve(sparse_A, dense_b):
        ...
        sparse_A = sparse_A.tocsc()

        # Form the normal equations: ATA = A^T A
        ATA = (sparse_A.T @ sparse_A).tocsc()  # Convert the result to CSC before passing it to splu()

        # Attempt LU factorization of ATA
        LU = scipy.sparse.linalg.splu(ATA, diag_pivot_thresh=0)

        # Build permutation matrices P1 and P2 from LU's permutation arrays.
        ...
        # Compute modified right-hand side: btilde = P1^T (A^T b)
        btilde = P1.T @ (sparse_A.T @ dense_b)

        # Solve the triangular systems:
        z = scipy.sparse.linalg.spsolve(LU.L, btilde)
        y = scipy.sparse.linalg.spsolve(LU.U, z)
        x = (P2.T @ y).toarray()
    ...
    def align(self, source_points, target_points, source_point_lmks=None, lmk_indices=None):
        ...
        for decay in range(3):
            print(f"Decay iteration: {decay}")
            cur_alpha = self.alpha * np.exp(-0.5 * decay)
            epsilon = self.epsilon * pow(0.5, decay)*6
            step = 0
            while True:
                ...
                X = self.spsolve(A, B)
                print("Solved system")
                # Compute the new source configuration using the current solution.
                new_src = cur_D * X
                new_src = new_src.transpose()

                # Debug: print norm before and after update, and the change in source points.
                norm_old = np.linalg.norm(cur_src)
                norm_new = np.linalg.norm(new_src)
                diff = new_src - cur_src
                print("Norm of cur_src before update:", norm_old)
                print("Norm of cur_src after update:", norm_new)
                print("Mean change in cur_src:", np.mean(np.abs(diff)))
                print("Max change in cur_src:", np.max(np.abs(diff)))

                # Update the current source configuration.
                cur_src = new_src.copy()
                step += 1
...
if __name__ == "__main__":
    from facebenchmark.performance_reporters.error_computation.rigid_aligners import ICPRigidAligner, \
        LandmarkBasedRigidAligner
    ...
    li = [19106, 19413, 19656, 19814, 19981, 20671, 20837, 20995, 21256, 21516, 8161, 8175, 8184, 8190, 6758, 7602,
          8201, 8802, 9641, 1831, 3759, 5049, 6086, 4545, 3515, 10455, 11482, 12643, 14583, 12915, 11881, 5522, 6154,
          7375, 8215, 9295, 10523, 10923, 9917, 9075, 8235, 7395, 6548, 5908, 7264, 8224, 9184, 10665, 8948, 8228, 7508]
    ...
    li68 = [748, 595, 1250, 1350, 1352, 1356, 1359, 1382, 1537, 1541, 1549, 1552, 1556, 1558, 602, 23, 78, 597, 1339, 4,
            1594, 720, 50, 1732, 0, 705, 25, 1474, 1478, 1439, 1480, 1075, 1112, 1476, 450, 407, 884, 863, 926, 1538,
            785, 825, 1705, 256, 239, 226, 117, 115, 1047, 1131, 1097, 1467, 435, 469, 379, 548, 587, 1441, 1245, 1206,
            1163, 1103, 1459, 441, 378, 582, 1447, 1240]
