...
parser.add_argument('dstmesh_path', type=str,
                    help="Path of the destination mesh. The created mesh will be written as a .txt file.")
...
if args.srcmesh_type not in supported_srcmesh_types:
    print(
        f'Given source mesh type is {args.srcmesh_type}, but we currently support only the following source mesh types: {supported_srcmesh_types}',
        file=sys.stderr)
    sys.exit(1)
