...
DEFAULT_OUT_DIR = REPO_ROOT / "checking_assumptions" / "outputs" / "mixed_icp_triplet_visuals"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Render clean vs high-mixed vs rigid-ICP triplet overlays.")
    ...
    p.add_argument("--mixed_sigma", type=float, default=0.20)
...
def _make_params(args: argparse.Namespace) -> base.PerturbationParams:
    ns = SimpleNamespace(
        outlier_frac=0.0,
        outlier_scale=0.0,
        jitter_sigma=float(args.mixed_sigma),
        translation_sigma=float(args.mixed_sigma),
        rotation_sigma=float(args.mixed_sigma),
        mixed_jitter_sigma=float(args.mixed_sigma),
        mixed_translation_sigma=float(args.mixed_sigma),
        mixed_rotation_sigma=float(args.mixed_sigma),
...
def _scenario_spec(args: argparse.Namespace) -> tuple[base.ScenarioSpec, int]:
    scenarios = base._parse_scenarios(
        "mixed",
        base_sigma=float(args.mixed_sigma),
...
def main() -> None:
    ...
    V_a_mix = base._apply_scenario(V=V_a, params=params, scenario=scenario, seed=seed_a).contiguous()
    V_b_mix = base._apply_scenario(V=V_b, params=params, scenario=scenario, seed=seed_b).contiguous()

    icp_idx_a = reg_utils.build_sample_vertex_indices(vertex_count=int(V_a_mix.shape[0]), n_points=int(args.icp_points), seed=idx_a * 97 + 11)
    icp_idx_b = reg_utils.build_sample_vertex_indices(vertex_count=int(V_b_mix.shape[0]), n_points=int(args.icp_points), seed=idx_b * 97 + 11)
    src_icp = reg_utils.extract_point_subset(V_a_mix, icp_idx_a)
    tgt_icp = reg_utils.extract_point_subset(V_b_mix, icp_idx_b)
    ...
    V_a_aligned = reg_utils.apply_rigid_transform(V_a_mix, transform)
    ...
    mix_src = _sample_for_plot(V_a_mix.detach().cpu().numpy(), n_points=int(args.plot_points), seed=idx_a * 17 + 3)
    mix_tgt = _sample_for_plot(V_b_mix.detach().cpu().numpy(), n_points=int(args.plot_points), seed=idx_b * 17 + 4)
    aligned_src = _sample_for_plot(V_a_aligned.detach().cpu().numpy(), n_points=int(args.plot_points), seed=idx_a * 17 + 5)

    title = (
        f"mixed sigma={float(args.mixed_sigma):.2f} | {args.sample_name_a} -> {args.sample_name_b}\n"
        f"clean vs target, mixed vs target, post-ICP vs target"
    )
    out_path = out_dir / f"mixed_sigma{float(args.mixed_sigma):.2f}_{args.sample_name_a}__to__{args.sample_name_b}.png"
    ...
    panels = [
        ...
        (axes[1, 0], mix_src[:, 0], mix_src[:, 1], mix_tgt[:, 0], mix_tgt[:, 1], "Mixed front (x,y)"),
        (axes[1, 1], mix_src[:, 2], mix_src[:, 1], mix_tgt[:, 2], mix_tgt[:, 1], "Mixed depth (z,y)"),
        (axes[2, 0], aligned_src[:, 0], aligned_src[:, 1], mix_tgt[:, 0], mix_tgt[:, 1], "Post-ICP front (x,y)"),
        (axes[2, 1], aligned_src[:, 2], aligned_src[:, 1], mix_tgt[:, 2], mix_tgt[:, 1], "Post-ICP depth (z,y)"),
    ]
    ...
    manifest = {
        ...
        "scenario": "mixed",
        "mixed_sigma": float(args.mixed_sigma),
    ...
    (out_dir / f"mixed_sigma{float(args.mixed_sigma):.2f}_{args.sample_name_a}__to__{args.sample_name_b}_manifest.json").write_text(
