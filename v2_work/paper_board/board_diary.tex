...
\section{18 agosto --- Ricerca di letteratura sui functional maps nell'AI}

Tredici paper rilevanti, quattro letti nel merito; resoconto completo in
\texttt{paper/REPORT.md} sezione 8. Tre esiti che cambiano qualcosa, e uno \`e scomodo.

\finding{\textbf{La normalizzazione d'area \`e la convenzione di preprocessing del campo, e noi
non la facevamo.} Bracha, Dag\`es \& Kimmel (ACCV 2024, arXiv:2310.14692), nella sezione
sperimentale: \emph{``as in other methods [4,14], we normalize the square root of the area of the
shapes to one''}. Cio\`e $\sqrt{A}=1$, esattamente ci\`o che fa
\texttt{areanorm\_operators.py} --- e lo citano come pratica \emph{altrui}, non come contributo
proprio. Normalizzare per \texttt{maxabs} non \`e una scelta alternativa documentata: \`e fuori
convenzione. \texttt{pot\_area} non \`e quindi una nostra idea, \`e un allineamento a uno
standard; pi\`u facile da difendere, e pi\`u imbarazzante da aver mancato.}

\finding{\textbf{Esiste un teorema per cui l'errore cresce con l'area mancante.} Gli stessi
autori dimostrano che l'errore del fmap layer sotto parzialit\`a \`e proporzionale all'area
mancante, e distinguono il proprio risultato da quello di Rodol\`a: \emph{``While the error in
[39] relates to the length of the cut, our analysis exhibits a relation to the missing area''}.
Due conti teorici indipendenti che predicono entrambi il nostro asse \texttt{crop}. La nostra
misura --- $\lambda_{128}$ che si sposta del $16\%$ sul crop contro il $17\%$ previsto da Weyl
sull'area persa del $14{,}6\%$ --- \`e l'istanza discreta del secondo.}

\paragraph{Perch\'e il pozzo era destinato a fallire, secondo il campo.} Luo \& Chen
(arXiv:2603.23383) elencano l'Hamiltoniano di Choukroun fra le variazioni specializzate della
eigenbase e concludono che \emph{``these methods are fundamentally constrained by their reliance
on axiomatic and fixed basis functions, which precludes the use of data-driven strategies''}. Il
nostro pozzo \`e un'istanza di una limitazione nota, non un incidente. Va accanto all'avvertimento
di Choukroun sull'amplificazione del rumore: due letture che, fatte prima, avrebbero risparmiato
l'esperimento. \`E la seconda volta oggi che la letteratura conteneva gi\`a la risposta.

\decision{Se si torna sul pozzo, si torna nella forma \emph{appresa}. Lo stesso paper sostituisce
la base fissa con $\Psi_k = \Phi_k e^{-T}$, $T$ diagonale di parametri appresi, e --- dettaglio
che conta --- \emph{``initialized to zero, ensuring that the inhibition function acts as an
identity mapping at the onset of training''}. \`E il pozzo, ma appreso e \textbf{inizializzato al
no-op}. Il nostro partiva da una perturbazione enorme ($c=10^{10}$, ROI che tratteneva il
$22$--$50\%$ della superficie): il contrario di questa disciplina.}

\paragraph{Tensione di progetto, dichiarata prima di provare qualcosa.} La loro diffusione \`e
deliberatamente \emph{eigenvalue-agnostic}: criticano il kernel standard perch\'e \emph{``the
eigenvalues serve as fixed weights for the diffusion time, high-frequency components are often
excessively suppressed regardless of their task-relevance''}. Ma eigenvalue-agnostic significa
appaiare per \textbf{indice}, e il teorema di Rodol\`a dice che sotto parzialit\`a l'appaiamento
per indice \`e quello sbagliato, perch\'e la diagonale \`e inclinata del rapporto di aree. Il loro
rimedio e la nostra normalizzazione d'area toccano la stessa riga di codice e tirano in direzioni
opposte sulla parzialit\`a; loro riportano guadagni su rumore topologico e casi non isometrici,
che non \`e la nostra parzialit\`a. \textbf{Adottarlo alla cieca disferebbe ci\`o che
\texttt{pot\_area} sta misurando.} Da decidere con un numero, non per analogia.

\paragraph{Limite del trasferimento, detto per non abusarne.} Tutta questa letteratura calcola
\emph{corrispondenze} e ha un fmap layer; noi calcoliamo un \emph{embedding globale} per una
metrica di ranking e non ne abbiamo uno. Trasferiscono la convenzione di preprocessing, la teoria
del perch\'e la parzialit\`a danneggia lo spettro troncato, e il meccanismo delle inhibition
functions. Non trasferisce la loro soluzione principale --- abbandonare il fmap layer per
matching diretto di feature con loss di Gromov --- perch\'e non abbiamo niente da abbandonare.

\paragraph{Nota sul frame.} Il difetto di \texttt{maxabs} misurato oggi (scala che si sposta di
$\pm 0{,}034$ per identit\`a sotto crop) non ha controparte in letteratura per la ragione
semplice che nessuno usa \texttt{maxabs}. \`E un difetto nostro, e serve \texttt{pot\_rms} per
quantificarlo.


\section{18 agosto --- Capacit\`a esclusa, e la strategia cambia}

\finding{\textbf{Il controllo di capacit\`a \`e negativo: la sconfitta del braccio senza
operatori non dipende dai parametri.}}

\begin{center}\small
\begin{tabular}{llrrrrr}
\toprule
braccio & backbone & parametri & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
\texttt{pot\_plain} & DiffusionNet   & $691{.}584$ & $0{,}7072$ & $0{,}7561$ & $0{,}7719$ & $0{,}7347$ \\
\texttt{pot\_point} & point $w{=}128$ & $288{.}768$ & $0{,}3155$ & $0{,}6536$ & $0{,}6684$ & $0{,}4659$ \\
\texttt{pt\_wide}   & point $w{=}256$ & $428{.}928$ & $0{,}3038$ & $0{,}6598$ & $0{,}6742$ & $0{,}4481$ \\
\bottomrule
\end{tabular}
\end{center}

Il $49\%$ di parametri in pi\`u non sposta nulla: su \texttt{crop} scende perfino di poco, e ogni
differenza sta dentro il rumore. Il controllo era stato registrato in anticipo, il giorno stesso,
come condizione per poter attribuire la sconfitta all'assenza di operatori invece che alla
capienza. Condizione soddisfatta. \textbf{Aggiungere parametri non sostituisce un sistema di
riferimento.}

\subsection*{Decisione di scope: le ablation sono uno strumento, non un prodotto}

\decision{Abbandonata la mia proposta di un paper breve sulle sole ablation. Tre ragioni, la
prima delle quali \`e un'obiezione di Leonardo che ha retto al controllo.}

\begin{enumerate}
\item \textbf{Erediterebbe la causa di rifiuto di v1.} Le review NeurIPS 2026 elencano
      ``perimetro sintetico'' fra le cause; un paper di sole ablation sullo stesso dataset
      sintetico ripete l'errore invece di ripararlo.
\item \textbf{Il peso di quel formato \`e misurabile.} Cercata la letteratura per verificare se
      l'ablation esistesse gi\`a: non nella nostra forma, ma esiste
      \texttt{arXiv:2606.07791} (\emph{Frequency-Scale Saliency for Spectral Descriptor
      Analysis}, giugno 2026), autore singolo, che fa lo stesso genere di lavoro --- ablation per
      bande su descrittori spettrali --- e dichiara da s\'e: \emph{``The primary limitation of
      this study is its reliance on a single benchmark''}. Quattro visualizzazioni. \`E la
      calibrazione di quanto pesa un paper di sole ablation. Nota utile: le sue future work
      nominano \emph{``learned spectral descriptors and surface networks''}, cio\`e il nostro
      territorio resta libero.
\item \textbf{Brucerebbe il risultato dove serve meno.} L'ablation \`e la risposta alla causa
      C4 (``backbone non giustificato'') e il piano di resubmission la aveva gi\`a prevista come
      sezione 2.2, ``ablation backbone come studio, non appendice''.
\end{enumerate}

\paragraph{Il ruolo vero delle ablation.} Non sono un risultato da pubblicare, sono uno
strumento che ha prodotto due indicazioni costruttive: il frame \`e rotto, la banda \`e rotta.
Cio\`e una \emph{ricetta} per un encoder che non si rompe sul crop. Il contributo non \`e
``abbiamo misurato che gli operatori servono'' ma ``abbiamo capito perch\'e il crop rompe e
costruito il modello che regge''.

\decision{Due sedi, due classi di contributo.
\textbf{CVPR} (deadline $\sim$novembre, tre mesi): paper di \emph{metodo} --- encoder di
superficie robusto alla topologia, ablation come evidenza, modello corretto come contributo. Un
paper di metodo sopravvive su dati controllati; era il paper \emph{benchmark} ad aver bisogno
dello human study e degli scan reali, ed \`e esattamente la differenza che ha fatto rifiutare v1,
che rivendicava una metrica senza validarla contro l'uso reale.
\textbf{ICCV 2027}: i tre pilastri del piano, benchmark e teoria della compressione, invariati.}

\paragraph{Il gate, dichiarato prima di vedere i numeri.} Tutto questo presuppone che il modello
corretto \emph{esista}, cio\`e che \texttt{pot\_area} o \texttt{pot\_rms} recuperino davvero
punti su \texttt{crop}. Se il $2\times2$ dice che nessuna delle due correzioni sposta la metrica,
non c'\`e un modello da portare a CVPR: c'\`e solo una diagnosi negativa, la terza di fila dopo
il pozzo e gli operatori, e la domanda diventa un'altra. Nessuna decisione di strategia prima di
quel numero.

\paragraph{Debito noto.} Il piano richiede tre seed e ne abbiamo uno. Per una differenza come
$0{,}7072$ contro $0{,}3155$ un seed basta; per $0{,}7072$ contro $0{,}7012$ no. Se il $2\times2$
d\`a un guadagno piccolo, va replicato prima di scriverci sopra qualcosa.


\section{18 agosto --- Il crop riparametrizza, e di quanto esattamente}

Idea di Leonardo: se il crop \emph{riparametrizza} lo spettro invece di distruggerlo, allora la
riparametrizzazione si misura e si correla con qualcosa. Si misura senza allenare nulla.

\finding{\textbf{Sotto crop un solo scalare spiega il $99{,}95\%$ dello spostamento dell'intero
spettro, e quel numero \`e il rapporto di aree.} Sessanta identit\`a, modi $1$--$128$, fit ai
minimi quadrati di $\lambda^t_k = \alpha_t \lambda^{\mathrm{orig}}_k$:}

\begin{center}\small
\begin{tabular}{lrrrrr}
\toprule
topologia & $\alpha$ (fit) & $A_{\mathrm{orig}}/A_t$ & $\alpha/$rapporto & $R^2$ & tilt residuo \\
\midrule
\texttt{crop}   & $1{,}1621 \pm 0{,}0095$ & $1{,}1748 \pm 0{,}0102$ & $0{,}9892 \pm 0{,}0011$ & $0{,}9995$ & $+0{,}0068$ \\
\texttt{noisy}  & $0{,}5004 \pm 0{,}0054$ & $0{,}4390 \pm 0{,}0064$ & $1{,}1399 \pm 0{,}0049$ & $0{,}9994$ & $-0{,}0073$ \\
\texttt{down8k} & $0{,}9893 \pm 0{,}0003$ & $1{,}0000 \pm 0{,}0004$ & $0{,}9893 \pm 0{,}0004$ & $0{,}9999$ & $-0{,}0061$ \\
\texttt{up60k}  & $1{,}0099 \pm 0{,}0005$ & $1{,}0050 \pm 0{,}0006$ & $1{,}0049 \pm 0{,}0004$ & $0{,}9999$ & $+0{,}0005$ \\
\texttt{remesh} & $1{,}0396 \pm 0{,}0013$ & $1{,}0389 \pm 0{,}0016$ & $1{,}0007 \pm 0{,}0007$ & $0{,}9996$ & $-0{,}0068$ \\
\bottomrule
\end{tabular}
\end{center}

Correlazione di Pearson fra $\alpha$ stimato e rapporto di aree: \texttt{crop} $+0{,}9932$,
\texttt{noisy} $+0{,}9846$, \texttt{remesh} $+0{,}9121$. Il residuo \`e \emph{piatto} in $k$
(tilt $\sim 0{,}007$), quindi non si tratta nemmeno di una riparametrizzazione dipendente
dall'indice: \`e un \textbf{riscalamento puro}, pi\`u pulito di quanto la diagonale inclinata di
Rodol\`a farebbe attendere per gli autovalori.

\finding{\textbf{L'eccezione \`e \texttt{noisy}, e insegna qualcosa.} $\alpha/$rapporto
$= 1{,}1399$: l'area geometrica sovrastima del $14\%$. Verificato per una via indipendente
stimando l'area \emph{spettrale} $A_{\mathrm{Weyl}} = 4\pi k/\lambda_k$ per regressione nella
banda di mezzo ($k = 20$--$110$):}

\begin{center}\small
\begin{tabular}{lrrr}
\toprule
topologia & $A_{\mathrm{geom}}/$orig & $A_{\mathrm{Weyl}}/$orig & Weyl$/$geom \\
\midrule
\texttt{crop}   & $0{,}8513 \pm 0{,}0075$ & $0{,}8611 \pm 0{,}0071$ & $1{,}0116 \pm 0{,}0012$ \\
\texttt{remesh} & $0{,}9626 \pm 0{,}0015$ & $0{,}9607 \pm 0{,}0012$ & $0{,}9980 \pm 0{,}0007$ \\
\texttt{up60k}  & $0{,}9950 \pm 0{,}0006$ & $0{,}9906 \pm 0{,}0005$ & $0{,}9955 \pm 0{,}0004$ \\
\texttt{noisy}  & $2{,}2785 \pm 0{,}0332$ & $1{,}9955 \pm 0{,}0217$ & $0{,}8758 \pm 0{,}0039$ \\
\bottomrule
\end{tabular}
\end{center}

Su crop e su tutte le topologie di ricampionamento l'area geometrica \`e fedele all'$1\%$ alla
superficie che il calore percepisce. Su \texttt{noisy} sbaglia del $14\%$ con dispersione
$\pm 0{,}004$, dunque sistematicamente: \textbf{il rumore aggiunge triangoli che ci sono e non
contano.} Due misure indipendenti danno lo stesso numero --- questo $14\%$ e l'overshoot del
$13\%$ trovato ieri sulla banda ($\lambda_{128}$ da $0{,}4966$ a $1{,}1315$ dopo normalizzazione
d'area).

\paragraph{Che cosa cambia, e che cosa non cambia.} Non ne esce una normalizzazione nuova da
provare subito. Ne esce che \textbf{la correzione di \texttt{pot\_area} \`e ora dimostrata quasi
esatta sul crop}: un solo scalare, $R^2 = 0{,}9995$, e quello scalare \`e il rapporto di aree
all'$1{,}1\%$ con l'area geometrica fedele all'$1{,}2\%$. Non resta margine di approssimazione in
cui rifugiarsi, e questo cambia il significato di entrambi gli esiti del gate:

\begin{itemize}
\item se \texttt{pot\_area} recupera punti su \texttt{crop}, sappiamo esattamente perch\'e;
\item se \emph{non} li recupera, non \`e pi\`u lecito dire ``forse la correzione era
      approssimativa''. Diventa: \emph{abbiamo corretto il difetto spettrale in modo esatto e la
      metrica non si \`e mossa} --- risultato negativo molto pi\`u forte, che sposterebbe
      l'imputazione sul frame, cio\`e su \texttt{pot\_rms}.
\end{itemize}

\decision{Normalizzazione ad ``area spettrale'' ($\lambda \cdot A_{\mathrm{Weyl}}$ invece di
$\lambda \cdot A_{\mathrm{geom}}$) messa da parte, malgrado sia la cosa pi\`u elegante uscita
oggi. L'unica topologia su cui differirebbe \`e \texttt{noisy}, che a $0{,}7561$ \`e il nostro
secondo asse migliore: sarebbe una correzione precisa applicata a qualcosa che non \`e rotto.
Diventa interessante solo se il gate passa e \texttt{noisy} resta indietro. Costo comunque
basso: sotto scala uniforme gli operatori si riscalano analiticamente ($\lambda \to \lambda/s^2$,
massa $\to s^2$, gradienti $\to 1/s$, autovettori invarianti), quindi non richiederebbe di
ricalcolare nulla.}


\section{18 agosto --- Il calore vede la stessa forma, ma il descrittore no}

Seconda idea di Leonardo: sistemare la scala perch\'e, guardata col calore, la versione crop
coincida con l'originale. Riformulata nella forma testabile come metodo --- a test time
l'originale non c'\`e, quindi ogni mesh deve normalizzarsi da sola --- e misurata con la traccia
del kernel del calore $\mathrm{Tr}(e^{-t\Delta}) = \sum_k e^{-\lambda_k t}$, che \`e globale e
non richiede vertici appaiati.

\paragraph{Prima, una correzione sul dataset.} Leonardo sospettava che \texttt{noisy} fosse
``distrutto'' e proponeva di toglierlo. Misurato: i vertici si spostano dello $0{,}48\%$ della
diagonale del volto in media, dell'$1{,}01\%$ al novantanovesimo percentile --- circa un
millimetro su venti centimetri, cio\`e rumore di scansione realistico. \textbf{La forma non \`e
toccata; \`e l'area a raddoppiare} ($2{,}283\times$ per triangolo), perch\'e il rumore \`e ad
alta frequenza e increspa ogni faccetta. \`E il paradosso della linea di costa sulla nostra
mesh, e rende \texttt{noisy} pi\`u prezioso, non meno: \`e la dimostrazione pi\`u pulita che
l'area geometrica \`e un cattivo normalizzatore, visto che una perturbazione invisibile la
raddoppia. Pi\`u il fatto pratico che con $0{,}7561$ \`e il nostro secondo asse migliore, quindi
toglierlo abbasserebbe il numero di testa. \decision{\texttt{noisy} resta.}

\finding{\textbf{Normalizzando ogni mesh col proprio spettro, il calore vede davvero la stessa
forma.} Quaranta identit\`a, $24$ tempi log-spaziati, taratura globale per convenzione:}

\begin{center}\small
\begin{tabular}{lrrrrr}
\toprule
convenzione & within & within \texttt{crop} & between & b/w & b/w \texttt{crop} \\
\midrule
\texttt{raw}  ($\lambda$)                       & $0{,}5791$ & $0{,}3329$ & $0{,}0835$ & $0{,}14$ & $0{,}25$ \\
\texttt{geom} ($\lambda A_{\mathrm{geom}}$)     & $0{,}1216$ & $0{,}0354$ & $0{,}0055$ & $0{,}04$ & $0{,}15$ \\
\texttt{weyl} ($\lambda A_{\mathrm{Weyl}}$)     & $0{,}0082$ & $0{,}0109$ & $0{,}0026$ & $0{,}31$ & $0{,}24$ \\
\bottomrule
\end{tabular}
\end{center}

Il \emph{within} --- distanza fra topologie della stessa identit\`a --- crolla di $71\times$ dal
grezzo alla convenzione di Weyl. L'obiettivo ``il calore vede la stessa forma'' \`e raggiunto.
Ma il \emph{between} crolla di $32\times$: il segnale d'identit\`a si contrae quasi quanto il
disturbo.

\finding{\textbf{Il crollo del \emph{between} non \`e colpa della normalizzazione, \`e colpa del
descrittore.} Misurato su $120$ identit\`a: l'area varia del $3{,}2\%$ \emph{fra identit\`a}
mentre il crop la sposta del $14{,}9\%$. Il disturbo di topologia sull'area \`e dunque
$4{,}6\times$ pi\`u grande del segnale d'identit\`a che vi risiede, e normalizzarla costa il
$3{,}2\%$ per togliere il $14{,}9\%$: scambio nettamente favorevole. Ne segue che la traccia del
calore --- ventiquattro numeri globali --- stava misurando soprattutto la \emph{taglia} del
volto, e una volta tolta la scala resta poco. Non \`e un'obiezione alla normalizzazione d'area:
\`e la prova che la traccia del calore \`e un pessimo descrittore d'identit\`a.}

\paragraph{Paletto contro me stesso, scritto prima che il numero arrivi.} Da questa tabella
verrebbe la tentazione di predire che \texttt{pot\_area} peggiorer\`a, visto che \texttt{geom}
ha rapporto $0{,}04$ contro lo $0{,}14$ del grezzo. \textbf{Non lo predico.} \`E esattamente la
forma dell'errore gi\`a commesso: lo sweep di $\alpha$ del pozzo migliorava del $46\%$ su un
proxy di questa stessa famiglia e il modello allenato peggior\`o sul gruppo interessato. Il proxy
misura la traccia del calore; DiffusionNet non usa la traccia del calore, usa feature apprese per
vertice con xyz in ingresso, e l'xyz la forma ce l'ha ancora. Se \texttt{pot\_area} migliorer\`a,
avremo la seconda dimostrazione indipendente che questa classe di proxy non predice la metrica ---
e sar\`a un risultato riportabile quanto il primo.

\paragraph{Errore di misura corretto in corsa.} Il primo tentativo dava distanze nulle sulla riga
\texttt{raw}: la finestra dei tempi era tarata per mesh ad area unitaria e sugli autovalori
grezzi la traccia satura, rendendo il descrittore una costante. Corretto con una taratura di un
solo numero per convenzione, calcolata sull'intero insieme --- globale, quindi senza reintrodurre
normalizzazione per-mesh dalla porta di servizio.


\section{18 agosto, notte --- Il gate passa: la normalizzazione d'area recupera il crop}

\finding{\textbf{$+2{,}2$ punti su \texttt{crop}, l'asse rotto.} Cento soggetti held-out,
$148{.}500$ coppie, ricetta, seed e regola di selezione identici a \texttt{pot\_plain}; unica
differenza gli operatori, calcolati su mesh ad area unitaria.}
...
braccio & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
\texttt{pot\_plain} & $0{,}7072$ & $0{,}7561$ & $0{,}7719$ & $0{,}7347$ \\
\texttt{pot\_area}  & $0{,}7292$ & $0{,}7699$ & $0{,}7575$ & $0{,}7432$ \\
\midrule
$\Delta$            & $+0{,}0220$ & $+0{,}0138$ & $-0{,}0144$ & $+0{,}0085$ \\
\bottomrule
\end{tabular}
\end{center}

Il guadagno cade dove la previsione meccanicistica lo collocava. Le tre misure di ieri e di oggi
--- $\lambda_{128}$ che si sposta del $16\%$ sotto crop come Weyl prevede, un solo scalare che
spiega il $99{,}95\%$ dello spostamento spettrale, quello scalare uguale al rapporto di aree
all'$1{,}1\%$ --- descrivevano un difetto specifico dell'asse \texttt{crop}, e correggendolo
\`e \texttt{crop} a salire. \textbf{Il gate \`e superato: il modello corretto esiste.}

\finding{\textbf{Seconda dimostrazione, registrata in anticipo, che i proxy senza training non
predicono la metrica.} Poche ore prima di questo numero la traccia del kernel del calore dava
alla convenzione \texttt{geom} un rapporto \emph{between}/\emph{within} di $0{,}04$ contro lo
$0{,}14$ del grezzo, cio\`e diceva che normalizzare per l'area \emph{peggiora} la separabilit\`a.
Era stato scritto nel diario, prima del risultato: ``da questa tabella verrebbe la tentazione di
predire che \texttt{pot\_area} peggiorer\`a. \textbf{Non lo predico.}'' \`E migliorato. Con lo
sweep di $\alpha$ del pozzo fanno due casi indipendenti in cui una misura spettrale senza
training punta nella direzione opposta al modello allenato.}

\paragraph{Il costo, che va riportato.} \texttt{resample} scende di $1{,}4$ punti, da $0{,}7719$
a $0{,}7575$. Sorprende, perch\'e per le topologie di ricampionamento la normalizzazione d'area
non cambia quasi nulla ($\alpha/$rapporto fra $0{,}989$ e $1{,}005$): non c'\`e un meccanismo che
preveda quel calo, il che lo rende un candidato a essere rumore di seed. Non lo si pu\`o
distinguere con una sola replica.

\decision{\textbf{Il gate passa in via provvisoria e non si scrive nulla in un paper prima delle
tre repliche.} Il criterio era gi\`a fissato la mattina: ``per $0{,}7072$ contro $0{,}3155$ un
seed basta, per un guadagno piccolo no''. Qui i guadagni sono $+0{,}022$ e $+0{,}0085$: piccoli.
Servono \texttt{pot\_plain} e \texttt{pot\_area} su tre seed prima di poter chiamare reale la
differenza, e prima di poter dire se il calo su \texttt{resample} \`e un effetto o rumore.}

\paragraph{Stato.} Quarta cella del $2\times2$ (\texttt{pot\_rms\_area}, frame rms su operatori
ad area unitaria) lanciata sulla GPU liberata da \texttt{pot\_area}; \texttt{pot\_rms} in corso
all'epoca $6/60$. Il $2\times2$ completo dir\`a se le due correzioni sono additive o se una
assorbe l'altra.


\section{19 agosto --- Il frame era il difetto maggiore}

\finding{\textbf{Il frame \texttt{rms} vale $2{,}6$ volte la normalizzazione d'area sull'asse
rotto, e migliora anche tutto il resto.} Stessi operatori di \texttt{pot\_plain}, stessa ricetta,
stesso seed, stessa regola di selezione: cambia solo il sistema di riferimento dei vertici, da
media-sui-vertici pi\`u \texttt{maxabs} a centroide pesato per massa pi\`u raggio quadratico
medio.}
...
\texttt{pot\_plain} & $0{,}7072$ & $0{,}7561$ & $0{,}7719$ & $0{,}7347$ \\
\texttt{pot\_area}  & $0{,}7292$ & $0{,}7699$ & $0{,}7575$ & $0{,}7432$ \\
\texttt{pot\_rms}   & $\mathbf{0{,}7637}$ & $\mathbf{0{,}7958}$ & $0{,}7664$ & $\mathbf{0{,}7697}$ \\
\midrule
$\Delta$ area       & $+0{,}0220$ & $+0{,}0139$ & $-0{,}0144$ & $+0{,}0086$ \\
$\Delta$ rms        & $+0{,}0566$ & $+0{,}0397$ & $-0{,}0055$ & $+0{,}0351$ \\
\bottomrule
\end{tabular}
\end{center}

Conferma diretta della misura di stabilit\`a del frame: sotto crop la scala \texttt{maxabs} si
spostava di $\pm 0{,}034$ \emph{in modo diverso per ogni identit\`a}, perch\'e dipende da quale
vertice fosse il pi\`u lontano e da quanto il taglio ne avesse mangiato. Quella dispersione era
rumore iniettato dritto nel confronto fra identit\`a, ed \`e la met\`a pi\`u grande del problema.
Notare che il frame \texttt{rms} migliora anche \texttt{noisy} di $+0{,}0397$ e riduce il costo su
\texttt{resample} da $-0{,}0144$ a $-0{,}0055$: non \`e uno scambio fra assi come lo era la
normalizzazione d'area, \`e un guadagno quasi ovunque.

\decision{Resta provvisorio: un seed. Il guadagno di $+0{,}0566$ \`e per\`o di ordine ben diverso
dal $+0{,}0220$ della normalizzazione d'area, quindi meno esposto al rumore di replica --- non
abbastanza da saltare le repliche, che sono in corso.}

\paragraph{Due difetti dell'automazione, trovati e corretti prima che producessero un numero
falso.}
\begin{enumerate}
\item \textbf{Collisione di glob.} \texttt{auto\_eval.sh} cercava
      \texttt{runs/\$\{tag\}\_*/*/checkpoints/epoch060.pth}, e \texttt{pot\_rms\_*} cattura anche
      \texttt{pot\_rms\_area\_*} --- come \texttt{pot\_plain\_*} cattura \texttt{pot\_plain\_s2}
      e \texttt{s3}. Il controllo ``il braccio ha finito'' diventava vero appena una qualsiasi
      delle omonime finiva, e \texttt{head -1} poteva prendere il checkpoint di un altro braccio:
      nessun errore sollevato, numeri plausibili. Corretto in \texttt{auto\_eval2.sh} con
      \texttt{\$\{tag\}\_[0-9]*}, dato che i run dir sono \texttt{<tag>\_<JOBID>} numerici. Il
      file non \`e stato modificato in luogo perch\'e era in esecuzione: bash rilegge da un
      offset spostato, difetto gi\`a pagato una volta.
\item \textbf{\texttt{pkill -f} ha ucciso la shell che lo eseguiva} (uscita $144$), perch\'e il
      pattern compare nella riga di comando del processo chiamante --- la stessa forma dell'errore
      gi\`a registrato per \texttt{pgrep -f}. Conseguenza silenziosa: il comando successivo della
      catena, il \texttt{sed} che alzava il parallelismo, non \`e mai stato eseguito, e senza
      controllare lo stato si sarebbe creduto fatto. Ora si uccide per PID.
\end{enumerate}


\section{19 agosto --- Il $2\times2$ chiuso: le due correzioni sono antagoniste}

\finding{\textbf{Insieme fanno peggio della sola migliore.}}

\begin{center}\small
\begin{tabular}{lrr}
\toprule
frame $\backslash$ operatori & attuali & area unitaria \\
\midrule
\texttt{current} & $0{,}7072$ & $0{,}7292$ \\
\texttt{rms}     & $\mathbf{0{,}7637}$ & $0{,}7377$ \\
\bottomrule
\end{tabular}
\qquad
\begin{tabular}{lrrrr}
\toprule
$\Delta$ vs controllo & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
solo area & $+0{,}0220$ & $+0{,}0139$ & $-0{,}0144$ & $+0{,}0086$ \\
solo rms  & $+0{,}0566$ & $+0{,}0397$ & $-0{,}0055$ & $+0{,}0351$ \\
entrambe  & $+0{,}0306$ & $+0{,}0280$ & $-0{,}0152$ & $+0{,}0170$ \\
\bottomrule
\end{tabular}
\end{center}

Non \`e sub-additivit\`a: \`e antagonismo. Applicare la normalizzazione d'area \emph{sopra} il
frame rms costa $-0{,}0260$ su \texttt{crop} rispetto al frame rms da solo.

\paragraph{Il fatto che rende la lettura non ambigua.} Il frame rms \`e invariante a traslazione
e scala uniforme --- dimostrato algebricamente e verificato nel suo self-check --- quindi l'xyz
che la rete vede in \texttt{pot\_rms\_area} \`e \textbf{bit-identico} a quello di
\texttt{pot\_rms}. L'unica differenza fra quei due bracci sono gli operatori. Ma l'unica
differenza fra \texttt{pot\_plain} e \texttt{pot\_area} sono, di nuovo, solo gli operatori, e l\`i
la normalizzazione d'area \emph{aiuta} di $+0{,}0220$. \textbf{Lo stesso identico cambiamento
negli operatori cambia segno a seconda del frame xyz.} Non \`e un effetto attribuibile al caso
sperimentale: \`e un'interazione fra i due canali di ingresso.

\decision{\textbf{Ipotesi registrata prima di costruire il braccio che la testa.} Con il frame
rms la rete riceve xyz in unit\`a di raggio quadratico medio e un Laplaciano in unit\`a di area:
due canali normalizzati da grandezze diverse. I tempi di diffusione appresi agiscono su
$\lambda$, che non \`e pi\`u coerente con la scala delle coordinate. \textbf{Previsione}: operatori
calcolati sulla stessa normalizzazione che la rete vede sull'xyz --- centroide pesato per massa,
raggio quadratico medio unitario --- devono fare almeno quanto \texttt{pot\_rms}. Se non lo
fanno, l'ipotesi della coerenza di unit\`a \`e sbagliata e va scritto.}

\paragraph{Costruzione, e il controllo che la rende sensata.} \texttt{rmsnorm\_operators.py}
normalizza ogni mesh col centroide pesato per le aree baricentriche e col raggio quadratico medio
prima di calcolare gli operatori. Le aree per vertice vanno ricostruite dalle facce, perch\'e la
\texttt{mass} salvata negli operatori \`e esattamente ci\`o che stiamo per calcolare e non esiste
ancora. Se le due definizioni divergessero, xyz e operatori tornerebbero su scale diverse, che \`e
il difetto che questo braccio esiste per rimuovere --- quindi \`e stato verificato: somma delle
aree identica a $10^{-8}$, e \textbf{raggio rms identico a $4\cdot10^{-10}$}, che \`e la quantit\`a
che fissa la scala.

\paragraph{Nota operativa.} Sette shard e non sei per la generazione: con sei topologie per
identit\`a e nomi ordinati, uno stride di sei assegna a un worker una topologia intera. \`E cos\`i
che \texttt{an5} si prese tutte le \texttt{up60k} da $60$k vertici e and\`o tre volte pi\`u lento
degli altri. Sette \`e coprimo con sei e le distribuisce: gli shard risultano da $428$--$429$
elementi ciascuno.

\paragraph{Sempre provvisorio.} Un seed. Le repliche \texttt{s2} sono in training e le
\texttt{s3} in coda. Il $-0{,}0260$ dell'antagonismo \`e per\`o pi\`u grande del $+0{,}0220$ che
stiamo gi\`a trattando come segnale, quindi o sono reali entrambi o nessuno dei due.


\subsection*{Tabella di runtime corretta nel paper}

\decision{Sostituiti i tempi degli operatori in \texttt{paper.tex}, sezione
\emph{Computational Cost}. Dichiarati: $1{,}2$--$2{,}15$~s per mesh da $16{,}5$k--$23{,}5$k
vertici e $6{,}3$~s per le $60{,}4$k. Misurati su nodo dedicato, mediana di tre prove:
$1{,}65$~s a $8{,}1$k, $3{,}70$~s a $16{,}5$k, $5{,}71$~s a $23{,}5$k, $15{,}65$~s a $60{,}4$k.
Aggiunti il comportamento in funzione dei thread ($5{,}82$, $6{,}22$, $5{,}44$, $5{,}31$,
$5{,}32$~s da 1 a 32 thread: piatto, perch\'e domina il Lanczos sparso), il forward di
DiffusionNet ($0{,}73$~s, mai misurato prima) e i $20{,}4$~MB di operatori per mesh.
Il paragrafo dichiara esplicitamente che i numeri precedenti erano sottostimati di circa un
fattore tre.}

Nella prima stesura avevo scritto che le vecchie misure erano state prese ``su una macchina
condivisa e congestionata''. \`E un'inferenza plausibile ma non la so, e in un paper una causa
inventata \`e peggio di nessuna causa: rimossa. Il paragrafo ora dice solo che i numeri erano
sbagliati e li corregge.

L'argomento di ammortamento resta intatto --- il costo \`e per-mesh e non per-coppia --- quindi
si correggono i numeri, non la struttura del claim. \texttt{paper.tex} compila, $20$ pagine.


\section{19 agosto, alba --- Ritrattazione: il rumore di seed \`e pi\`u grande dell'effetto}

\finding{\textbf{Lo stesso braccio, lo stesso codice, la stessa ricetta, solo il seed diverso:}}

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
\texttt{pot\_plain} & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
seed $1234$ & $0{,}7072$ & $0{,}7561$ & $0{,}7719$ & $0{,}7347$ \\
seed $1235$ & $0{,}7333$ & $0{,}7977$ & $0{,}7857$ & $0{,}7641$ \\
\midrule
differenza  & $0{,}0261$ & $0{,}0416$ & $0{,}0138$ & $0{,}0294$ \\
\bottomrule
\end{tabular}
\end{center}

\decision{\textbf{RITIRO l'affermazione ``il gate \`e passato'' scritta ieri sera.} Il guadagno
che avevo attribuito alla normalizzazione d'area era $+0{,}0220$ su \texttt{crop}; la dispersione
fra due seed dello stesso identico braccio \`e $0{,}0261$. L'effetto era pi\`u piccolo del rumore.
Contro la media dei due seed di controllo, \texttt{pot\_area} vale $+0{,}0090$ su \texttt{crop} e
\textbf{negativo} su \texttt{noisy}, \texttt{resample} e \texttt{all}.}

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
$\Delta$ contro controllo a 2 seed & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
\texttt{pot\_area}     & $+0{,}0090$ & $-0{,}0069$ & $-0{,}0213$ & $-0{,}0061$ \\
\texttt{pot\_rms}      & $+0{,}0435$ & $+0{,}0189$ & $-0{,}0124$ & $+0{,}0203$ \\
\texttt{pot\_rms\_area}& $+0{,}0175$ & $+0{,}0072$ & $-0{,}0221$ & $+0{,}0023$ \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Che cosa resta in piedi, e con quanta forza.} \texttt{pot\_rms} \`e l'unico braccio
sopra il rumore: $+0{,}0435$ su \texttt{crop}, cio\`e circa $1{,}7$ volte la differenza fra due
seed del controllo, e $+0{,}0203$ su \texttt{all}. Non \`e ancora una misura --- ha un seed solo.

\paragraph{Il buco nel mio piano, detto per intero.} La catena di repliche era stata decisa
quando i bracci erano due, e replicava \texttt{pot\_plain} e \texttt{pot\_area}. \texttt{pot\_rms}
\`e arrivato dopo, si \`e rivelato il migliore, ed \`e rimasto senza repliche \emph{proprio il
braccio su cui poggia la conclusione}. Errore di pianificazione, non di esecuzione: le repliche
vanno decise per il braccio che vince, non per quelli che c'erano quando si \`e scritto lo script.
Corretto: \texttt{pot\_rms\_s2} e \texttt{pot\_rms\_s3} accodati.

\paragraph{Sul metodo, ed \`e la lezione pi\`u cara della notte.} Il criterio ``un seed basta per
$0{,}7072$ contro $0{,}3155$, non per un guadagno di $0{,}02$'' era stato scritto la mattina,
prima di qualunque numero, ed \`e servito: ha impedito che $+0{,}0220$ finisse in un paper. Ma
l'ho comunque \emph{annunciato} come un gate superato, con un entusiasmo che i dati non
sostenevano. Marcare un risultato provvisorio e poi parlarne come definitivo \`e il modo in cui
un caveat smette di funzionare.

\paragraph{Terza variante dello stesso difetto di shell, in una notte.} Dopo \texttt{pgrep -f}
che trova se stesso e \texttt{pkill -f} che uccide la shell che lo esegue, il controllo di salute
del monitor cercava \texttt{auto\_eval} con un pattern che \textbf{compare nella riga di comando
del monitor stesso}: si autotrovava, e non avrebbe mai potuto segnalare l'evaluator morto ---
che nel frattempo lo era davvero, ucciso dal \texttt{kill} di un comando precedente. Ora il
controllo \`e ancorato sul nome del programma e non su una sottostringa qualsiasi.


\section{19 agosto --- Correzione della ritrattazione: il denominatore era sbagliato}

\decision{La ritrattazione di un'ora fa confrontava il guadagno della normalizzazione d'area
contro la dispersione \emph{non appaiata} dei due seed del controllo. \`E il denominatore
sbagliato. \texttt{pot\_plain} e \texttt{pot\_area} con lo stesso seed condividono i pesi
iniziali e l'ordine dei dati: l'effetto del seed \`e \textbf{comune ai due bracci}, e la
differenza appaiata lo elimina. Usare la varianza non appaiata gonfia l'incertezza della
differenza e fa sembrare rumore un effetto reale.}

\finding{\textbf{Differenze appaiate, \texttt{pot\_area} contro \texttt{pot\_plain}:}}

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
 & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
seed $1234$ & $+0{,}0220$ & $+0{,}0139$ & $-0{,}0144$ & $+0{,}0086$ \\
seed $1235$ & $+0{,}0141$ & $+0{,}0074$ & $+0{,}0107$ & $+0{,}0120$ \\
\midrule
media       & $+0{,}0181$ & $+0{,}0106$ & $-0{,}0019$ & $+0{,}0103$ \\
spread      & $0{,}0079$ & $0{,}0065$ & $\mathbf{0{,}0251}$ & $0{,}0035$ \\
segno concorde & s\`i & s\`i & \textbf{NO} & s\`i \\
\bottomrule
\end{tabular}
\end{center}

Su \texttt{crop} l'effetto \`e $2{,}3$ volte la variabilit\`a della differenza appaiata, non
$0{,}7$ volte come appariva col denominatore sbagliato; su \texttt{all} \`e circa tre volte. Su
\texttt{resample} il segno \textbf{si inverte} fra i due seed, quindi il ``costo di $-0{,}0144$
su resample'' riportato ieri sera \`e rumore e non un effetto --- va ritirato anche quello, in
direzione opposta.

\paragraph{Due correzioni in senso contrario nella stessa notte, e non mi piace.} Ho annunciato
un gate con troppa sicurezza, l'ho ritirato con troppa severit\`a, e ora ne correggo met\`a.
L'errore comune alle due volte non \`e nei dati: \`e che ho scelto la statistica \emph{dopo} aver
visto i numeri invece di fissarla col disegno sperimentale. Un disegno con seed condivisi fra
bracci \`e appaiato per costruzione, e questo si sapeva quando la catena di repliche \`e stata
scritta, non dopo.

\decision{\texttt{aggregate\_2x2.py} calcola e stampa le differenze appaiate per seed, la loro
media, il loro spread e la \textbf{concordanza di segno}, con la nota che un segno discorde
significa assenza di effetto. Con $n$ piccolo la concordanza di segno dice pi\`u della media, e
la statistica giusta va nello strumento, non nella testa di chi guarda la tabella.}

\paragraph{Stato reale delle conclusioni, a due seed.} La normalizzazione d'area aiuta,
modestamente e con segno concorde su \texttt{crop}, \texttt{noisy} e \texttt{all}; non costa nulla
su \texttt{resample}. \texttt{pot\_rms} ha ancora un seed solo, quindi il suo $+0{,}0566$ resta
non appaiabile e non confrontabile con questi: le sue repliche sono in coda ed erano il buco di
pianificazione gi\`a registrato.


\section{19 agosto --- Normalizzazione d'area, tre repliche: effetto piccolo e non significativo}

\finding{\textbf{Tre differenze appaiate complete, \texttt{pot\_area} contro \texttt{pot\_plain}:}}
...
seed $1234$ & $+0{,}0220$ & $+0{,}0139$ & $-0{,}0144$ & $+0{,}0086$ \\
seed $1235$ & $+0{,}0141$ & $+0{,}0074$ & $+0{,}0107$ & $+0{,}0120$ \\
seed $1236$ & $+0{,}0508$ & $-0{,}0093$ & $+0{,}0037$ & $+0{,}0185$ \\
\midrule
media       & $+0{,}0290$ & $+0{,}0040$ & $-0{,}0000$ & $+0{,}0130$ \\
d.s.        & $0{,}0193$ & $0{,}0119$ & $0{,}0129$ & $0{,}0050$ \\
segno       & concorde & \textbf{discorde} & \textbf{discorde} & concorde \\
$t$ ($2$ g.l.) & $+2{,}60$ & $+0{,}58$ & $-0{,}00$ & $+4{,}49$ \\
$p$         & $0{,}121$ & $0{,}620$ & $0{,}997$ & $0{,}046$ \\
\bottomrule
\end{tabular}
\end{center}

\decision{\textbf{Formulazione definitiva per la normalizzazione d'area: guadagno piccolo,
coerente in segno su \texttt{crop} e sul punteggio aggregato, e \emph{non significativo} a questa
numerosit\`a.} Il $p = 0{,}046$ su \texttt{all} \`e nominalmente sotto $0{,}05$, ma i gruppi
testati sono quattro e con la correzione di Bonferroni servirebbe $p < 0{,}0125$: non
sopravvive. Su \texttt{noisy} e \texttt{resample} non c'\`e effetto --- il segno cambia fra
repliche. Va riportato cos\`i, non come una vittoria.}

\paragraph{Il limite duro di $n=3$.} Il test dei segni con tre campioni concordi d\`a
$p = 1/8 = 0{,}125$, che \`e il valore \emph{minimo ottenibile}: tre punti non possono dare di
pi\`u, quale che sia la dimensione dell'effetto. Chiedere tre repliche era il minimo per
distinguere segnale da rumore, non abbastanza per dimostrarlo. Se questo numero deve entrare in
un paper come claim e non come osservazione, servono pi\`u repliche o un disegno con pi\`u
potenza --- il che va deciso ora, non in revisione.

\paragraph{Traiettoria di questa conclusione, che vale quanto la conclusione.} In dodici ore lo
stesso effetto \`e stato chiamato ``il gate \`e passato'' ($+0{,}0220$, un seed), poi ritirato
come rumore (confronto contro varianza non appaiata), poi corretto a effetto reale (differenze
appaiate, due seed), e ora misurato come piccolo e non significativo (tre seed, con correzione
per confronti multipli). Ogni passaggio era difendibile con i dati del momento; il difetto \`e
stato annunciare i primi due con la sicurezza che solo l'ultimo poteva permettersi. La regola
che ne segue non \`e ``aspetta sempre'', ma: \textbf{dichiarare la numerosit\`a e il test
insieme al numero, ogni volta}, cos\`i che il grado di fiducia sia leggibile senza doverlo
dedurre dal tono.

\decision{\texttt{aggregate\_2x2.py} stampa ora, per ogni gruppo, media, deviazione standard,
concordanza di segno, $t$ con i gradi di libert\`a, $p$, e la nota che il test dei segni non pu\`o
scendere sotto $0{,}125$ con tre campioni. Lo strumento dice quanto vale il numero nello stesso
punto in cui lo dice.}

\paragraph{Resta aperto il braccio che conta.} \texttt{pot\_rms} ha ancora un seed, con
$+0{,}0566$ su \texttt{crop} --- il doppio dell'effetto dell'area. \texttt{pot\_rms\_s2} \`e in
training e \texttt{pot\_rms\_s3} in coda; solo allora si potr\`a dire se il frame \`e davvero
l'intervento maggiore o se anche quel numero si sgonfia con le repliche.


\section{19 agosto --- Ipotesi confermata, e la storia si semplifica}

\finding{\textbf{Allineare le unit\`a rimuove l'antagonismo, e non aggiunge altro.} Tutti i
bracci al seed $1234$, quindi direttamente confrontabili:}

\begin{center}\small
\begin{tabular}{llrrrr}
\toprule
braccio & operatori & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
\texttt{pot\_plain}     & $L$, frame corrente        & $0{,}7072$ & $0{,}7561$ & $0{,}7719$ & $0{,}7347$ \\
\texttt{pot\_rms}       & $L$, frame rms             & $\mathbf{0{,}7637}$ & $\mathbf{0{,}7958}$ & $0{,}7664$ & $\mathbf{0{,}7697}$ \\
\texttt{pot\_rms\_area} & $L$ area unitaria $+$ rms  & $0{,}7377$ & $0{,}7841$ & $0{,}7567$ & $0{,}7516$ \\
\texttt{pot\_rmsops}    & $L$ raggio rms $+$ rms     & $0{,}7601$ & $0{,}7900$ & $0{,}7707$ & $0{,}7666$ \\
\bottomrule
\end{tabular}
\end{center}

La previsione registrata prima di costruire il braccio era: \emph{``operatori calcolati sulla
stessa normalizzazione che la rete vede sull'xyz devono fare almeno quanto \texttt{pot\_rms}''}.
Misurato $0{,}7601$ contro $0{,}7637$, cio\`e $-0{,}0036$: un quarto della dispersione di seed
del controllo ($0{,}0140$), dunque indistinguibili. La previsione regge.

E soprattutto la penalit\`a di $-0{,}0260$ di \texttt{pot\_rms\_area} \`e \textbf{sparita}:
l'antagonismo era incoerenza di unit\`a, xyz in raggio quadratico medio contro Laplaciano in
area, e allinearli lo risolve.

\decision{\textbf{Lettura definitiva, e semplifica la storia invece di complicarla: una volta
sistemato il frame xyz, la normalizzazione degli operatori non conta pi\`u.} Le tre varianti col
frame rms stanno entro $0{,}0036$--$0{,}0260$ l'una dall'altra, e le due che condividono le
unit\`a sono indistinguibili. \`E il frame a fare tutto il lavoro. Per il paper significa
\emph{una riga sola di preprocessing}, non due che si ostacolano.}

\paragraph{Nota di cautela sulla numerosit\`a, che vale per tutta questa tabella.} Ogni cella
qui \`e un singolo seed. Il confronto \texttt{pot\_rms} contro \texttt{pot\_rmsops}
($-0{,}0036$) \`e molto sotto il rumore e non distingue nulla; quello contro
\texttt{pot\_rms\_area} ($-0{,}0260$) \`e circa il doppio del rumore e quindi plausibilmente
reale, ma resta n$=1$. Le repliche di \texttt{pot\_rms} sono in training.


\section{19 agosto --- Sintesi: tutti i difetti vivono nella scala dell'xyz}

\finding{Osservazione di Leonardo, che unifica due risultati che avevo trattato come separati.
Rimessi in fila, \textbf{ogni difetto misurato vive nella scala del canale delle coordinate}, e
nessuno negli operatori:}

\begin{center}\small
\begin{tabular}{lll}
\toprule
difetto & natura & costo \\
\midrule
frame \texttt{maxabs} & scala xyz dipendente dal bordo & $-0{,}057$ su \texttt{crop} \\
operatori area $+$ xyz rms & xyz e $L$ su scale diverse & $-0{,}026$ su \texttt{crop} \\
operatori rms $+$ xyz rms & coerenti & recuperato \\
\bottomrule
\end{tabular}
\end{center}

Gli operatori non sono mai stati il problema in nessuno dei tre casi. Ci\`o che era rotto \`e il
canale delle coordinate, due volte: una in \emph{assoluto} --- la scala dipende da quale vertice
il crop ha rimosso --- e una in \emph{relativo}, incoerente con gli operatori. Sono lo stesso
difetto visto da due angoli, non due scoperte.

\paragraph{Dove la sintesi non arriva, e come si testa.} Da qui la forma forte
dell'inferenza sarebbe: se l'xyz \`e il punto d'ingresso del danno, non va dato affatto. Non
segue. Che un canale sia il punto d'ingresso del danno implica che vada \emph{inquadrato} bene,
non che vada tolto --- ed \`e esattamente ci\`o che \texttt{pot\_rms} fa. La domanda ha per\`o una
risposta sperimentale, con codice gi\`a scritto e mai eseguito perch\'e finito sulle code
pubbliche.

\decision{Lanciato \texttt{abl\_intrinsic}: \texttt{--no-use\_xyz --n\_hks 16 --n\_wks 16}, cio\`e
DiffusionNet senza alcuna coordinata, solo descrittori intrinseci sullo spettro. Ricetta
identica a \texttt{pot\_plain} in tutto il resto --- stessi dati, frame corrente, seed $1234$,
$60$ epoche --- quindi il confronto isola il solo canale di ingresso.}

\paragraph{Previsione registrata prima del lancio.} Il braccio intrinseco sar\`a
\emph{relativamente} pi\`u robusto su \texttt{crop}, non avendo un frame di coordinate da
rompere, ma \textbf{nettamente peggiore su \texttt{all}}: l'identit\`a di un volto \`e in buona
parte estrinseca --- proporzioni, profondit\`a, sporgenze --- e i descrittori intrinseci sono
invarianti alle isometrie, cio\`e per costruzione non distinguono volti che differiscono per
come sono piegati nello spazio. Due misure gi\`a acquisite la sostengono: togliendo gli operatori
e lasciando solo xyz \texttt{crop} crolla a $0{,}3155$, e FLAME ha consistenza spettrale
$3{,}5\times$ migliore di BFM con ranking molto peggiore ($0{,}478$ contro $0{,}751$) --- pi\`u
invarianza intrinseca non ha mai significato metrica migliore, in questo progetto.

\textbf{Se invece \texttt{abl\_intrinsic} batte \texttt{pot\_plain} su \texttt{all}}, la
previsione \`e sbagliata, l'ipotesi nella forma forte \`e giusta, e la direzione del lavoro
cambia: non riparare il frame ma togliere le coordinate. Scritto nello script del lancio, cos\`i
che non sia attenuabile a posteriori.


\section{19 agosto --- Il frame regge alla replica, l'area molto meno}

\finding{\textbf{Due differenze appaiate per il frame rms, contro tre per la normalizzazione
d'area:}}
...
braccio & & \texttt{crop} & \texttt{noisy} & \texttt{resample} & \texttt{all} \\
\midrule
\texttt{pot\_rms}  & media  & $\mathbf{+0{,}0517}$ & $+0{,}0324$ & $-0{,}0010$ & $\mathbf{+0{,}0306}$ \\
($2$ seed)         & spread & $\mathbf{0{,}0098}$ & $0{,}0146$ & $0{,}0089$ & $0{,}0089$ \\
                   & segno  & concorde & concorde & discorde & concorde \\
\midrule
\texttt{pot\_area} & media  & $+0{,}0290$ & $+0{,}0040$ & $-0{,}0000$ & $+0{,}0130$ \\
($3$ seed)         & spread & $0{,}0366$ & $0{,}0232$ & $0{,}0251$ & $0{,}0099$ \\
                   & segno  & concorde & discorde & discorde & concorde \\
\bottomrule
\end{tabular}
\end{center}

Su \texttt{crop} l'effetto del frame \`e $1{,}8\times$ pi\`u grande e $3{,}7\times$ pi\`u
consistente: spread $0{,}0098$ su un effetto di $0{,}0517$, cio\`e un quinto. La normalizzazione
d'area ha spread $0{,}0366$ su un effetto di $0{,}0290$, dunque \emph{pi\`u grande dell'effetto
stesso}. Il frame guadagna anche su \texttt{noisy} con segno concorde, dove l'area cambia
direzione fra repliche. Su \texttt{resample} nessuno dei due fa nulla.

\decision{Con $n=2$ non si calcola alcun test: \texttt{pot\_rms\_s3} \`e in training. Ma la
gerarchia fra i due rimedi \`e ormai netta e va nella direzione opposta all'ordine in cui li
abbiamo scoperti --- la normalizzazione d'area, che era l'ipotesi di partenza e ha una teoria
elegante dietro (Weyl, Rodol\`a, la convenzione del campo), \`e il rimedio debole; il frame, che
\`e emerso da una misura banale di stabilit\`a, \`e quello forte.}

\paragraph{Proporzione, per non ripetere l'errore di ieri.} Questo \`e un guadagno
\emph{in-domain}. Il divario che conta \`e altrove: $0{,}7506$ su BFM held-out contro
$\mathbf{0{,}0571}$ su FaceVerse. Il $+0{,}0306$ sul punteggio aggregato \`e numericamente
mezzo punto in pi\`u del valore assoluto della prestazione fuori dominio. Va riportato per
quello che \`e: una correzione di preprocessing reale e riproducibile, non la soluzione del
problema del progetto.

\paragraph{\texttt{abl\_intrinsic} morto per CUDA OOM.} Non un bug nostro: il nodo aveva un
processo altrui a $41{,}14$~GiB e la scheda a $67$~GB su $79$ con sedici job. \`E il braccio pi\`u
esigente in memoria perch\'e \texttt{intrinsic\_dn} calcola HKS e WKS su $300$ autovalori. Il
monitor l'ha rilevato su due segnali indipendenti --- scomparso dalla coda senza
\texttt{epoch060}, e traceback nella log --- che \`e esattamente perch\'e ce ne sono due.
Non rilanciato subito: scheda satura, e la priorit\`a si \`e spostata sui dati.


\section{19 agosto --- Il bersaglio, non il modello: il frame del ground truth}

Su indicazione di Leonardo, spostato il lavoro dal modello ai dati. Il primo bersaglio \`e il
ground truth stesso, che avevamo misurato dipendere dalla topologia in cui viene calcolato.

\finding{\textbf{Accordo del ground truth con se stesso attraverso le topologie}, stesse $500$
identit\`a misurate in topologia BFM ($23{.}470$ vertici) e FLAME ($1{.}930$):}

\begin{center}\small
\begin{tabular}{llr}
\toprule
frame & definizione & $\rho$ \\
\midrule
\texttt{raw}        & nessuna normalizzazione & $\mathbf{0{,}8652}$ \\
\texttt{global\_rms}& centroide per-mesh, scala costante per topologia & $0{,}7976$ \\
\texttt{area}       & centroide pesato, $\div \sqrt{A}$ & $0{,}7959$ \\
\texttt{rms}        & centroide pesato, $\div$ raggio quadratico medio & $0{,}7906$ \\
\texttt{maxabs}     & media dei vertici, $\div$ vertice pi\`u lontano & $0{,}5734$ \emph{(attuale)} \\
\bottomrule
\end{tabular}
\end{center}

I valori di \texttt{raw} e \texttt{maxabs} riproducono esattamente quelli gi\`a in
\texttt{agreement.json} ($0{,}8653$ e $0{,}5734$), il che valida lo script contro l'originale.

\paragraph{Ipotesi registrata e smentita.} Avevo previsto che il vantaggio di \texttt{raw}
venisse dalla \emph{taglia assoluta} del volto, che \`e informazione biometrica e che ogni frame
per-mesh cancella. Il frame \texttt{global\_rms} esiste per separare le due spiegazioni: centra
ogni mesh ma divide per una costante unica, quindi preserva le differenze di taglia fra
identit\`a. Se l'ipotesi fosse giusta avrebbe dovuto valere quanto \texttt{raw}; vale quanto
\texttt{rms}. \textbf{Smentita.}

\finding{La causa si isola per algebra, non per congettura: dividere tutte le mesh di una
topologia per la stessa costante non pu\`o cambiare i ranghi, quindi lo Spearman \`e invariante.
L'unica differenza rimasta fra \texttt{raw} e \texttt{global\_rms} \`e la \textbf{centratura}.
Dunque \emph{centrare ogni mesh sul proprio centroide fa perdere segnale}, per lo stesso
meccanismo di tutto il resto: il centroide si calcola su insiemi di vertici diversi nelle due
topologie, e centrare inietta una perturbazione topologia-dipendente. \`E \texttt{maxabs} in
forma pi\`u debole.}

\decision{\textbf{La raccomandazione \`e il compromesso, non l'ottimo.} La decomposizione del
guadagno recuperabile \`e:
\[
\texttt{maxabs} \to \texttt{global\_rms}: +0{,}2242 \quad (77\%), \qquad
\texttt{global\_rms} \to \texttt{raw}: +0{,}0676 \quad (23\%).
\]
Il grosso viene dall'abbandonare \texttt{maxabs}. E il bersaglio \texttt{raw} \textbf{non \`e
utilizzabile}: dipende dalla posizione assoluta di ogni mesh, che l'ingresso del modello --
centrato per necessit\`a --- non contiene; chiederemmo alla rete di predire qualcosa che non pu\`o
vedere. Si adotta quindi centroide pesato per area pi\`u scala fissa, che recupera tre quarti
della perdita ed \`e coerente con un frame d'ingresso realmente usabile --- la stessa lezione di
coerenza di unit\`a emersa da \texttt{pot\_rmsops}.}

\paragraph{Proporzione.} Il guadagno sul frame d'ingresso, misurato ieri, valeva $+0{,}0517$ su
\texttt{crop}. Questo vale $+0{,}22$ di auto-consistenza \emph{del bersaglio}, cio\`e un tetto su
tutto ci\`o che qualunque modello pu\`o raggiungere. \`E lo stesso difetto --- la normalizzazione
per-mesh dipendente dal bordo --- applicato all'altro lato del problema, e sull'altro lato pesa
quattro volte tanto.

\paragraph{Cosa non risolve, detto subito.} Il ground truth resta \emph{geometrico e non
biometrico}: nessuna etichetta d'identit\`a entra nel pipeline, quindi la circolarit\`a (causa
C2) non \`e toccata. E non dice nulla sul divario fuori dominio ($0{,}7506$ contro $0{,}0571$).
Rende il bersaglio meno rumoroso; non lo rende la cosa giusta da misurare.


\section{31 agosto --- Ritrattazione: il bersaglio non era normalizzato, e cinque frame convivevano}

\paragraph{Che cosa avevo affermato.} Nella sezione del 19 agosto ho scritto che $D_{\mathrm{GT}}$
\`e «calcolato dopo la normalizzazione per-mesh del loader», che \texttt{maxabs} costa
$0{,}22$ di auto-consistenza \emph{al bersaglio}, e che il bersaglio \texttt{raw} «non \`e
utilizzabile» perch\'e l'ingresso del modello \`e «centrato per necessit\`a». \textbf{Tutte e tre
le affermazioni sono false}, e la terza era un errore di ragionamento, non di misura.

\paragraph{L'origine dell'errore.} Ho creduto al docstring invece che al codice.
\texttt{compute\_gt\_distance\_matrix\_normalized.py} dichiara di applicare «the SAME
normalisation applied in GTReadyDatasetNPZ: subtract mean, divide by global max |coord|».
Il codice non lo fa: \texttt{load\_mesh} restituisce \texttt{verts} grezzi, e l'unica
normalizzazione \`e \texttt{D / max\_val} sulla matrice delle distanze --- una costante, che non
cambia nessun rango.

\finding{Verificato per confronto diretto con la colonna \texttt{gt\_distance} effettivamente
usata, su tutte le $148{.}500$ coppie:
\[
\rho(\text{salvato}, \texttt{raw}) = 1{,}000000,
\]
contro $0{,}8163$ per \texttt{global\_rms}, $0{,}7951$ per \texttt{area}, $0{,}7909$ per
\texttt{rms}, $0{,}7854$ per \texttt{maxabs}. \textbf{Il bersaglio \`e \texttt{raw}}, cio\`e il
migliore dei cinque frame, non il peggiore. Non \`e mai stato danneggiato.}

\paragraph{Il difetto vero: cinque frame in una tabella.}

\begin{center}\small
\begin{tabular}{lll}
\toprule
quantit\`a & frame & dove \\
\midrule
$D_{\mathrm{GT}}$ (bersaglio) & \texttt{raw} & \texttt{compute\_gt\_distance\_matrix\_normalized.py} \\
ingresso del modello & \texttt{maxabs} per-mesh & \texttt{dataset\_gtready.py:167-171, 299-306} \\
\texttt{raw\_chamfer} & \texttt{maxabs} per-mesh & \texttt{mesh\_npz\_utils.py:17-22} \\
varifold, currents & \texttt{area} per-mesh & \texttt{measure\_distances.py:116-118} \\
arcface, clip, dinov2, lpips & bbox della camera & \texttt{render\_mesh.py} \\
rigid/NICP (in pi\`u) & \textbf{per-coppia} & \texttt{icp.py:109-116} \\
\bottomrule
\end{tabular}
\end{center}

Il paper (riga 278--279) afferma: «All meshes are additionally normalized per mesh \emph{before
any distance is computed}, by centering on the vertex mean and dividing by the largest absolute
coordinate». \`E falso per il bersaglio e per tre delle cinque righe.

\paragraph{Prima buona notizia: la classifica non dipende dal frame.} Ricalcolate le otto
metriche contro $D_{\mathrm{GT}}$ ricostruito in cinque frame
(\texttt{v2\_work/xdomain/frame\_metric\_ranking.py}), l'ordine \`e identico ovunque, tranne un
solo scambio sotto \texttt{maxabs} dovuto a \texttt{lpips}, che ha valori su $5{.}700$ coppie
su $148{.}500$. \textbf{La conclusione della Tabella~1 non \`e un artefatto della
normalizzazione.}

\paragraph{Seconda, meno buona: il margine \`e gonfiato.} Allontanandosi da \texttt{raw}, la
nostra metrica \emph{perde} ($0{,}7506 \to 0{,}6792$) mentre arcface, varifold e currents
\emph{guadagnano}. Siamo gli unici allenati contro \texttt{raw}. E \texttt{raw\_chamfer}, che
vive in \texttt{maxabs}, vale $0{,}2852$ nel proprio frame contro lo $0{,}2373$ che riportiamo:
la baseline \`e penalizzata di $0{,}048$ da una scelta nostra.

\section{31 agosto --- La compressione \`e sovrastimata, e l'ICP rigido quasi non c'entra}

\finding{Il paper misura la compressione col rapporto di IQR \textbf{assoluti}. Ma lo Spearman
\`e invariante per riscalamento: la parte del calo di IQR dovuta a un cambio di unit\`a non pu\`o
causare il degrado di ranking che dovrebbe spiegare. Le mediane calano al $79/65/58\%$ di raw,
quindi il cambio di unit\`a c'\`e. Il coefficiente di variazione, invariante, \emph{era gi\`a
calcolato} dal codice (\texttt{analyze\_distance\_compression.py:73}) e mai riportato.
\[
\begin{array}{lccc}
\text{metrica} & \text{IQR/raw (paper)} & \text{(IQR/med)/raw} & \text{CV/raw} \\
\hline
\text{Rigid ICP P2P} & 0{,}537 & 0{,}681 & 0{,}739 \\
\text{NICP P2P} & 0{,}219 & 0{,}339 & 0{,}364 \\
\text{NICP P2Tri} & 0{,}236 & 0{,}408 & 0{,}433
\end{array}
\]
Il paper scrive «NICP P2P retains only 21.9\%»; il valore invariante \`e $33{,}9\%$. Sovrastima
$1{,}55\times$.}

\paragraph{Il colpevole ha un nome.} \texttt{prealign\_by\_bbox} (\texttt{icp.py:109-116})
riscala la sorgente al raggio del \emph{bersaglio}: per la coppia ordinata $(A,B)$ la
trasformazione dipende da $B$, quindi cambia a ogni coppia. \`E applicata a \texttt{rigid\_p2p} e
a entrambe le NICP (\texttt{run\_facebench\_remesh.py:166-170, :185}) e \textbf{non} a
\texttt{raw\_chamfer} (\texttt{:158-161}).

\paragraph{Il controllo.} La pipeline esplorativa (\texttt{registration\_utils.py}) fa ICP
dall'identit\`a, senza prealign. Sulle stesse $30$ coppie di topologie:
\[
\begin{array}{lcc}
 & \text{senza prealign} & \text{con prealign (paper)} \\
\hline
\text{raw chamfer} & 0{,}3433 & 0{,}7295 \\
\text{rigid ICP} & 0{,}3065\ (-0{,}037) & 0{,}5995\ (-0{,}130) \\
\text{NICP} & 0{,}2146\ (-0{,}129) & 0{,}4561\ (-0{,}273) \\
\text{CPD} & 0{,}0110\ (-0{,}332) & \text{---}
\end{array}
\]
Senza prealign, l'ICP rigido peggiora il ranking in $17$ coppie su $30$: il $57\%$,
indistinguibile dal caso.

\decision{Il titolo va scomposto. \textbf{La registrazione non rigida danneggia il ranking}
--- NICP $-0{,}129$, CPD $-0{,}332$ anche senza prealign: il claim regge ed \`e grosso.
\textbf{L'ICP rigido quasi no}: tre quarti del danno attribuito vengono dal
\texttt{prealign\_by\_bbox}, che \`e una scelta implementativa di faceBench, non una propriet\`a
dell'ICP. Caveat onesto: i due numeri non sono perfettamente confrontabili (set di coppie e
aggregazione diversi), quindi la direzione regge, il fattore $3{,}5$ preciso no.}

\section{31 agosto --- Il frame globale: il bersaglio si preserva esattamente}

\finding{La soluzione non \`e scegliere un frame per-mesh migliore, \`e non usarne uno per-mesh.
Con una \textbf{singola similarit\`a globale} $V \mapsto (V - c_0)/s_0$, con $c_0, s_0$ costanti
dell'insieme, si ha $d(i,j) \mapsto d(i,j)/s_0$: un fattore costante, quindi \textbf{nessun rango
cambia}. Verificato su $60$ identit\`a eval, $1{.}770$ coppie
(\texttt{v2\_work/xdomain/global\_frame.py}):
\[
\rho(\text{raw}, \text{globale}) = 1{,}0000000000, \qquad
\text{rapporto costante entro } 6{,}66\times 10^{-16}.
\]
E l'ingresso resta allenabile: maxabs mediano $1{,}57$, span $3{,}03$. Sullo stesso insieme,
\texttt{maxabs} per-mesh perde $0{,}2243$.}

\decision{\textbf{Ritratto la raccomandazione del 19 agosto.} Avevo scelto centroide-area pi\`u
scala fissa perch\'e credevo che \texttt{raw} fosse irraggiungibile, dato che l'ingresso \`e
«centrato per necessit\`a». Il ragionamento era sbagliato: l'ingresso dev'essere \emph{a scala
sana}, non \emph{centrato per-mesh}. Una traslazione globale soddisfa la prima e non la seconda.
Il frame globale domina tutte e tre le alternative --- \texttt{rms}, \texttt{area},
\texttt{global\_rms} costano fra $0{,}068$ e $0{,}075$ al bersaglio, il globale costa zero.
$c_0$ e $s_0$ si stimano \textbf{solo sulle 400 identit\`a di training}: sono statistiche dei
dati, e calcolarle sulle 500 sarebbe leakage.}

\paragraph{Lezione, la stessa di ieri con un altro vestito.} Due volte di fila ho attribuito un
difetto alla componente sbagliata: prima agli operatori invece che alla normalizzazione, ora al
bersaglio invece che al disallineamento fra bersaglio e ingresso. In entrambi i casi la
correzione \`e arrivata da un controllo che \emph{poteva fallire} --- l'assert su
\texttt{maxabs} ricalcolato contro la colonna salvata --- e non da un ragionamento pi\`u
attento. Il codice va letto, non il docstring che lo descrive.


\section{31 agosto, sera --- Ritrattazione: il prealign non era il colpevole}

\paragraph{Che cosa avevo affermato stamattina.} Che «tre quarti del danno attribuito all'ICP
rigido vengono dal \texttt{prealign\_by\_bbox}», e che quindi il claim del paper sull'ICP rigido
era in gran parte un artefatto implementativo. Avevo dichiarato il caveat giusto --- «i due
numeri non sono perfettamente confrontabili, set di coppie e aggregazione diversi» --- e poi gli
ho dato troppo poco peso, riportando comunque il fattore $3{,}5$ come se fosse una misura.

\paragraph{L'esperimento appaiato.} Stesse coppie, stesso ICP, stessa normalizzazione della
pipeline, unica variabile il prealign acceso o spento
(\texttt{v2\_work/xdomain/prealign\_paired.py}, 25 identita' di valutazione, 4096 punti).

\finding{\textbf{Su 30 coppie di topologie} (9.000 coppie di mesh):
\[
\begin{array}{lcc}
 & \rho & \text{danno} \\
\hline
\text{raw chamfer} & 0{,}3540 & \\
\text{rigid ICP} + \text{prealign} & 0{,}3262 & -0{,}0278 \\
\text{rigid ICP senza prealign} & 0{,}3217 & -0{,}0323
\end{array}
\]
Il prealign non fa differenza, e il segno e' pure rovesciato: senza, e' leggermente peggio.

\textbf{Su original$\to$original}, cioe' il setting della tabella del paper (300 coppie di
soggetti; il raw e' $0{,}6013$ e non $0{,}7295$ perche' qui le identita' sono 25 e non 100,
quindi contano i \emph{delta}, che sono appaiati, non i valori assoluti):
\[
\begin{array}{lcc}
\text{raw chamfer} & 0{,}6013 & \\
\text{rigid ICP} + \text{prealign} & 0{,}4311 & -0{,}1702 \\
\text{rigid ICP senza prealign} & 0{,}4664 & -0{,}1349
\end{array}
\]
Il prealign vale $0{,}0353$ su $0{,}1702$, cioe' il \textbf{21\%}, non il 75\%. E fuori dal
setting a topologia fissa non vale nulla.}

\decision{\textbf{Il claim del paper sull'ICP rigido regge.} L'ICP rigido toglie $0{,}135$ di
Spearman a topologia fissa anche senza alcun prealign: e' la registrazione, non
l'implementazione. Va corretto solo il contorno --- dichiarare che la pipeline applica un
riscalamento per-coppia e che ne vale circa un quinto --- non la sostanza.

\textbf{E il danno dipende fortemente dal setting}, cosa che il paper non dice: $-0{,}135$ dove
il chamfer e' forte (topologia fissa, raw $0{,}60$), $-0{,}032$ dove il chamfer e' gia' debole
(cross-topologia, raw $0{,}35$). La registrazione distrugge in proporzione a quanto c'era da
distruggere. E' un'osservazione piu' interessante di quella che avevo sostituito, e va nel
paper.}

\paragraph{L'errore, e non e' nuovo.} Ho confrontato due numeri prodotti da pipeline diverse su
insiemi di coppie diversi con aggregazioni diverse, ho notato che non erano confrontabili, l'ho
scritto, e poi ho riportato lo stesso il rapporto fra i due. Il caveat scritto non annulla il
numero sbagliato: se una misura non e' confrontabile, non va riportata affatto finche' non lo
diventa. Il disegno appaiato costava due ore ed era disponibile fin dall'inizio.

Terza volta in due giorni che una componente viene incolpata al posto di un'altra: prima gli
operatori invece della normalizzazione, poi il bersaglio invece del disallineamento, ora il
prealign invece della registrazione. Le prime due volte la correzione e' arrivata da un assert;
questa da un esperimento appaiato. Nessuna delle tre da un ragionamento piu' attento.


\section{Prossimi passi}
\begin{enumerate}
\item \textbf{Frame globale nel loader}: sostituire la normalizzazione per-mesh con
      $(V-c_0)/s_0$ e riallenare. \`E l'unico cambio che allinea ingresso e bersaglio a costo
      zero sul bersaglio. Braccio \texttt{pot\_globalframe}, tre seed contro \texttt{pot\_plain}.
\item \textbf{Correzione della tabella di compressione}: riportare CV o IQR/mediana. La frase
      «does not merely rescale distances» diventa dimostrata invece che asserita.
\item \sout{Rimisurare l'ICP rigido senza \texttt{prealign\_by\_bbox}}. \textbf{Fatto,
      appaiato}: il prealign vale il 21\% a topologia fissa e nulla cross-topologia. Resta da
      riportare nel paper che il danno da registrazione dipende dal setting.
\item \textbf{Ripunteggiare le baseline nel proprio frame}: \texttt{raw\_chamfer} vale $0{,}2852$
      in \texttt{maxabs} contro lo $0{,}2373$ riportato. La baseline va accreditata del suo
      valore migliore, o il confronto \`e ingiusto.
\item \textbf{Correggere il paper} (riga 278--279): l'affermazione sulla normalizzazione \`e
      falsa. Serve approvazione, come da accordo del 20 agosto.
\item \textbf{Azione umana richiesta}: richieste d'accesso a Notre Dame CVRL, FaceScape,
      BU-3DFE; IRB/etica per lo human study.
\end{enumerate}
