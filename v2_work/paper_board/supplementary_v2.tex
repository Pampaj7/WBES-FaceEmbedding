...
\begin{tabular}{lcccc}
\toprule
 & no well & well misplaced & well $\alpha{=}0.55$ & \;+ masked pooling \\
Group & ($L$) & (per-mesh offset) & (placed) & (support fixed) \\
\midrule
crop (boundary moved)     & \textbf{0.7072} & 0.7034 & 0.7012 & \texttt{[pending]} \\
noisy                     & 0.7561 & 0.7573 & \textbf{0.7653} & \texttt{[pending]} \\
resample (sampling only)  & \textbf{0.7719} & 0.7533 & 0.7521 & \texttt{[pending]} \\
\midrule
all (pooled)              & \textbf{0.7347} & 0.7315 & 0.7215 & \texttt{[pending]} \\
\bottomrule
\end{tabular}
\end{table}
