#!/usr/bin/env bash
# Launch one FLAME training run on a GPU node, with the RAM/device cache.
#
#   bash launch_flame.sh <tag> <batch_subjects> <lr> [extra trainer flags...]
#
# The recipe is otherwise the v1 top-model recipe verbatim, so a run with
# batch_subjects=5 lr=1e-4 is the controlled counterpart of the v1 checkpoint
# (the cache changes where bytes come from, not the math).
set -euo pipefail
ROOT=/dtu/p1/leopam/WBES-FaceEmbedding
TAG=${1:?tag}; BS=${2:?batch_subjects}; LR=${3:?lr}; shift 3

cat > "$ROOT/v2_work/fastio/_node_${TAG}.sh" <<NODE
#!/bin/bash
set -u
cd $ROOT
...
NODE
chmod +x "$ROOT/v2_work/fastio/_node_${TAG}.sh"
