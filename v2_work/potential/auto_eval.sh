...
evaluate () {
  ...
  local ck
  ck=$(ls v2_work/runs/${tag}_*/*/checkpoints/best_by_xtopo_mesh_clean.pth 2>/dev/null | head -1)
...
while true; do
  ...
  while IFS='|' read -r tag data extra; do
    ...
    if ls v2_work/runs/${tag}_*/*/checkpoints/epoch060.pth >/dev/null 2>&1; then
