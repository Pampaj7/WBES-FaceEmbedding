...
exec .conda_env/bin/python v2_work/fastio/train_fast.py \
  --cache-residency ram --cache-workers 8 --frame global \
  ...
  --runs_root v2_work/runs/SMOKE_globalframe_${LSB_JOBID:-local} \
  --device cuda --model xyz_dn \
  --latent_dim 256 --width 128 --n_blocks 4 --dropout 0.1 \
  --pool_mode meanmax --k_spec 0 --no-log_spec --eig_k 300 \
  --epochs 1 --batch_subjects 5 --lr 1e-4 --weight_decay 1e-6 --grad_clip 1.0 \
  ...
  --eval_mode average --eval_every 1 --save_every 5 --seed 1234
